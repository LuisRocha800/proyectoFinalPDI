%%
% FUNCIÓN: "crearCollage".
% Crea un collage usando nueve imagenes. 
% 
% Input:
%       imgProcesada: matriz de la imagen procesada (actual)
%       img 2 a 9 :   matrices de las imagenes a usar en el collage
% Output:
%       collage: matriz del collage resultante

function collage = crearCollage(imgProcesada, img2, img3, img4, img5, img6, img7, img8, img9)

% Excepciones
if (isempty(imgProcesada) || isempty(img2) || isempty(img3) || isempty(img4) ...
        || isempty(img5) || isempty(img6) || isempty(img7) || isempty(img8) || isempty(img9))
    ME = MException('Image:Empty', 'SE DEBEN ABRIR LAS 9 IMAGENES EN LOS CAMPOS CORRESPONDIENTES.');
    throw(ME)
end
